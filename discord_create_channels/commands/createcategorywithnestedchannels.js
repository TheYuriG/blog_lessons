...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('createcategorywithnestedchannels') // Command name matching file name
	...
	async execute(interaction) {
		...
		try {
			...
			// If we managed to create the category and the nested channels,
			// edit the initial response with a success message
			await interaction.editReply({
				content: 'Your category and their nested channels were successfully created!',
			});
		} catch (error) {
			// If an error occurred and we were not able to create the category
			// or the nested channels, the bot is most likely received the
			// "Missing Permissions" error. Log the error to the console
			console.log(error);
			...
			await interaction.editReply({
				content:
					'Your category and their nested channels could not be created! Please check if the bot has the necessary permissions!',
			});
		}
	},
};
